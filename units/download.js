...
const wrongLinuxOSMsg = "Only Ubuntu 16.04 and 18.04 have precompiled binaries, on other linux systems you need to build the daemon yourself. Reffer to: https://github.com/w2w-coin/w2w";
const wrongOSMsg = "This operating system has no precompiled binaries you need to build the daemon yourself. Reffer to: https://github.com/w2w-coin/w2w";
...
module.exports = {
  downloadLatestDaemon: function (nodePath, callback) {
    ...
    downloadRelease('W2W', 'w2w', finalTempDir, filterRelease, filterAssetNode, true)
      .then(function () {
  ...
  downloadLatestW2wNode: function (callback) {
    var finalTempDir = path.join(tempDir, utils.ensureNodeUniqueId());
    ...
    // Define a function to filter assets.
    var filterAssetW2wNode = function (asset) {
      if (process.platform === "win32") {
        return asset.name.indexOf('win64') >= 0;
      } else if (process.platform === "linux") {
    ...
    downloadRelease('W2W', 'w2w-node', finalTempDir, filterRelease, filterAssetW2wNode, true)
      .then(function () {
        fs.readdir(finalTempDir, function (err, items) {
          if (items.length > 0) {
            extractArchive(path.join(finalTempDir, items[0]), finalTempDir, function (success) {
              if (success) {
                ...
                var executableName = utils.getW2wNodeExecutableName();
                ...
                shell.mv(path.join(process.cwd(), utils.getW2wNodeExecutableName()), path.join(process.cwd(), backupName));
