...
W2W Node is a process that monitors the `w2wd` daemon. It is catching daemon errors, monitors the block count and, in case of an error, it restarts the daemon and notifies on Discord via web hook, sends an e-mail, or both.
...
W2W Node can be installed to run with Node.js or used as precompiled binary.
...
```bash
$ git clone https://github.com/w2w-coin/w2w-node.git
$ cd w2w-node
$ npm install
```
...
Please refer to [installation page](INSTALL.md) for detailed instructions how to install the W2W Node with precompiled binaries.

## Configuration

The W2W Node can be configured manually by creating `config.json` file in the project's root directory or by using interactive setup (refer to [installation page](INSTALL.md) for instructions how to run interactive setup).
...
```
{
   "node":{
      ...
      "path":"C:\\Wallets\\W2W-CLI\\w2wd.exe",
      "port":21016,
      "name":"TestNode",
      "bindAddr":"0.0.0.0",
      "feeAddr":"your fee W2W address"
   },
   "error":{
      "notify":{
         "discord":{
         ...
         "email":{
            ...
            "message":{
               "from":"no-reply@w2wcoin.org",
               "to":"test.email@gmail.com",
               "subject":"W2W Node error occured"
            }
         }
      }
   },
   ...
   "pool":{
      "notify":{
         "url":"URL to the w2w node pool service if you want to register there",
         "interval":30
      }
   },
   "url": {
...
* **node**
  ...
  * path: The path of the process. If omited it uses the same path where the W2W Node is located
  * port: The port on which w2wd is running
  * name: Name of the node. If omited it uses the hostname.
  * feeAddr: The W2W address on which the transaction fee will be sent in case of remote node.
...
* **restart**
  ...
  * maxCloseErrors: Maximum number of errors. After that the W2W Node stops as there is a serious issue with the daemon.
...
* **api**
  * port: port of the api on which to listen. If not specified the W2W Node will not listen for incoming requests
* **pool**
  * **notify**
    * url: the url of the W2W Node Pool. The W2W Node is sending its data to pool for public listing
...
Depending on how W2W Node is installed, there are 2 ways to run it, with Node.js or by using precompiled binaries. In both cases, you can install it as a system service and run it that way too.
...
To run as a service you can use the build in service controls described on [installation page](https://github.com/w2w-coin/w2w-node/blob/master/SETUP.md). Or you can do it the manual way. On linux you can use **systemctl**
=======
...
ExecStart=/usr/bin/node /path/to/w2w-node/index.js
Restart=always
RestartSec=3
...
Edit the file accordingly and save it as `/etc/systemd/system/w2w-node.service`.

To enable the service and run it on startup, run:

```bash
$ sudo systemctl daemon-reload
$ sudo systemctl enable w2w-node.service
```
...
```bash
# start
$ sudo systemctl start w2w-node

# stop
$ sudo systemctl stop w2w-node

# status
$ sudo systemctl status w2w-node

# print log
$ journalctl -e -u w2w-node.service
...
<plist version="1.0">
  <dict>
    ...
    <array>
      ...
      <string>/path/to/w2w-node/index.js</string>
    </array>
...
Edit the file accordingly and save it as `~/Library/LaunchAgents/w2w-node.plist`.

Start it with:

```bash
launchctl load ~/Library/LaunchAgents/w2w-node.plist
```

If you want to unload it, run:

```bash
launchctl unload ~/Library/LaunchAgents/w2w-node.plist
```

### Precompiled binaries

Please refer to [installation page](INSTALL.md) for detailed instructions how to run the W2W Node with precompiled binaries.

## API

The W2W Node has an option to run HTTP server providing some general info about the node.  
...
```
{
   ...
   "blockchain":{
      ...
      "fee_address":"W2WcHrVi9nSeqktPVf6ztyLoqHKn5FET62z4QwfkqGPjGwobKUAMuGhCo39MPmLanYXzq2UnekWJtGgvijMLwBy15U3JCbtiZe",
